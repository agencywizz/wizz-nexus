...
def banner():
    print(f"""
{GREEN}  ╔══════════════════════════════════╗
  ║   {BOLD}WizzOS — Setup Wizard{RESET}{GREEN}     ║
  ╚══════════════════════════════════╝{RESET}
""")
...
def _check_tool(name, cmd, install_cmd=None, install_label=None, min_version=None):
    ...
    if install_cmd:
        ...
        if choice in ("", "y", "yes", "s", "sim"):
            ...
            print(f"  {RED}✗{RESET} {name} is required for WizzOS")
    else:
...
### Me
{config['owner_name']} — {config['company_name']} ({config['company_website']})
...
def copy_env_example(config: dict):
    ...
    if src.exists():
        ...
        dst.write_text("# WizzOS Environment Variables\n# Fill in your API keys below\n\n", encoding="utf-8")
...
def copy_routines_config(config: dict):
    ...
    if src.exists():
        ...
        dst.write_text("# WizzOS Routines — edit schedules here\n# See ROUTINES.md for documentation\n\ndaily: []\nweekly: []\nmonthly: []\n", encoding="utf-8")
...
def _setup_systemd_service(service_user, install_dir, logs_dir):
    """Create and start a systemd service for WizzOS."""
...
def main():
    ...
    if is_remote:
        ...
        owner_name = ""
        company_name = "Wizz! comms."
        company_website = "wizzcomms.com"
        ...
        company_name = ask("Company name", "Wizz! comms.")
        company_website = ask("Company website", "wizzcomms.com")
    ...
    if os.getuid() == 0 and not sudo_user and is_remote:
        ...
        service_home = f"/home/{service_user}"
        service_dir = f"{service_home}/wizz-os"
    ...
    os.system("systemctl stop wizz-os 2>/dev/null")
...
""", encoding="utf-8")
    ...
    if is_remote:
        ...
        if service_user == "evonexus":
            svc_msg = f"""
  Servico systemd:
    {DIM}systemctl status wizz-os{RESET}     — verificar status
    {DIM}systemctl restart wizz-os{RESET}    — reiniciar
    {DIM}journalctl -u wizz-os -f{RESET}     — ver logs
...
if __name__ == "__main__":
    ...
    # or the `npx @agencywizz/wizz-os` CLI, which sets EVO_NEXUS_INSTALL=1),
